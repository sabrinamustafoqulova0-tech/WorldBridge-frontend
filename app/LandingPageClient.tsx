...
export default function LandingPageClient({
  ...
  initialStats: { countries: number; programs: number };
}) {
  const { isAuthenticated, isLoading } = useAuthStore();
  const { lang } = useLangStore();
  const [mounted, setMounted] = useState(false);
  const [slideIndex, setSlideIndex] = useState(0);
  const [dbStats, setDbStats] = useState(initialStats);
  ...
  // Fetch fresh stats on mount just in case
  useEffect(() => {
    Promise.all([
    ...
    ]).then(([countriesCount, programsCount]) => {
      setDbStats({
        countries: countriesCount || initialStats.countries,
        programs: programsCount || initialStats.programs,
      });
    });
  }, [initialStats]);
